...
Here, we perform an exhaustive permutation test (with the t-statistic) and plot the results
...
We first present the data used
...
Then we run the permutation test, get the t-statistics and calculate the correponding p-value
...
We have the t-statistics as `r tobs` and the p-value being `r pval` which is `r ifelse(pval<0.05, "less than", "larger than")` 5%.

Finally, we plot the results in the following figure:
